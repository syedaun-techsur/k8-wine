...
| **Assess** | Edit page loads; sees quantity field showing 1 (or 0); decides to delete rather than set to 0 | F2 — Edit Bottle Page (`/bottles/[id]/edit`) | "I could set quantity to 0, but that leaves a ghost entry. Better to delete it." | Deliberate | No visual nudge toward delete when quantity is 0 or 1 | **Post-MVP:** If quantity field is set to 0 on save, suggest deletion ("Remove this bottle?"). Explicitly deferred — saving quantity=0 is valid in MVP; user must manually choose to delete. |
