...
LIST_RESPONSE=$(curl -s --max-time 15 "$BASE/api/bottles")
LIST_STATUS=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/api/bottles")
...
ROOT_STATUS=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/")
...
# Single POST — capture both response body and status code in one request
ADD_OUTPUT=$(curl -s --max-time 15 -X POST \
  -H "Content-Type: application/json" \
  -d '{"name":"Caymus","vintage":2019,"varietal":"Cabernet Sauvignon","quantity":3,"location":"Rack A3"}' \
  -w "\nHTTP_STATUS:%{http_code}" \
  "$BASE/api/bottles")
ADD_RESPONSE=$(echo "$ADD_OUTPUT" | sed '/^HTTP_STATUS:/d')
ADD_STATUS=$(echo "$ADD_OUTPUT" | grep "^HTTP_STATUS:" | cut -d: -f2)
...
AFTER_ADD_LIST=$(curl -s --max-time 15 "$BASE/api/bottles")
...
if [ -n "$BOTTLE_ID" ]; then
  ...
  GET_BOTTLE_STATUS=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/api/bottles/$BOTTLE_ID")
  GET_BOTTLE=$(curl -s --max-time 15 "$BASE/api/bottles/$BOTTLE_ID")
  ...
  AFTER_EDIT_LIST=$(curl -s --max-time 15 "$BASE/api/bottles")
...
if [ -n "$BOTTLE_ID" ]; then
  ...
  DELETE_STATUS=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" -X DELETE "$BASE/api/bottles/$BOTTLE_ID")
  ...
  AFTER_DELETE_LIST=$(curl -s --max-time 15 "$BASE/api/bottles")
...
" 2>/dev/null || echo "false")"
  ...
  AFTER_DELETE_GET=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/api/bottles/$BOTTLE_ID")
...
SEARCH_ADD=$(curl -s --max-time 15 -X POST \
  -H "Content-Type: application/json" \
...
SEARCH_STATUS=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/api/bottles?q=cay")
SEARCH_RESPONSE=$(curl -s --max-time 15 "$BASE/api/bottles?q=cay")
...
SEARCH_UPPER=$(curl -s --max-time 15 "$BASE/api/bottles?q=CAY")
...
SEARCH_EMPTY=$(curl -s --max-time 15 "$BASE/api/bottles?q=")
...
[ -n "$SEARCH_BOTTLE_ID" ] && curl -s --max-time 10 -X DELETE "$BASE/api/bottles/$SEARCH_BOTTLE_ID" > /dev/null || true
echo ""
...
PERSIST_ADD=$(curl -s --max-time 15 -X POST \
  -H "Content-Type: application/json" \
...
PERSIST_LIST=$(curl -s --max-time 15 "$BASE/api/bottles")
...
if [ -n "$PERSIST_ID" ]; then
  PERSIST_GET=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/api/bottles/$PERSIST_ID")
  uat_check "US6: GET /api/bottles/$PERSIST_ID still returns 200 (PostgreSQL persisted)" "$([ "$PERSIST_GET" = "200" ] && echo true || echo false)"
  # Clean up
  curl -s --max-time 10 -X DELETE "$BASE/api/bottles/$PERSIST_ID" > /dev/null || true
fi
echo ""
