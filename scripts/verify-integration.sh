...
HEALTH_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/api/health" || echo "000")
HEALTH_BODY=$(curl -s "$BASE/api/health" || echo "")
...
ALL_HEADERS=$(curl -s -I "$BASE/" || echo "")
FRAME_HEADER=$(echo "$ALL_HEADERS" | grep -i 'x-frame-options' | tr -d '\r' || echo "")
check "X-Frame-Options is not DENY" "$(echo "$FRAME_HEADER" | grep -qi 'DENY' && echo false || echo true)"
check "X-Frame-Options is SAMEORIGIN or absent" "$(if echo "$FRAME_HEADER" | grep -qi 'SAMEORIGIN' || [ -z "$FRAME_HEADER" ]; then echo true; else echo false; fi)"
CSP_HEADER=$(echo "$ALL_HEADERS" | grep -i 'content-security-policy' | tr -d '\r' || echo "")
...
LIST_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/api/bottles" || echo "000")
...
NOT_FOUND_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/api/bottles/99999999" || echo "000")
...
MIGRATE_OUT1=$(DATABASE_URL="${DATABASE_URL:-postgres://postgres:devpass@localhost:5432/app}" node scripts/migrate.mjs 2>&1 || echo "MIGRATE_FAILED")
EXIT1=$?
MIGRATE_OUT2=$(DATABASE_URL="${DATABASE_URL:-postgres://postgres:devpass@localhost:5432/app}" node scripts/migrate.mjs 2>&1 || echo "MIGRATE_FAILED")
EXIT2=$?
...
ROOT_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/" || echo "000")
NEW_STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/bottles/new" || echo "000")
...
NOTFOUND_EDIT=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/bottles/99999/edit" || echo "000")
