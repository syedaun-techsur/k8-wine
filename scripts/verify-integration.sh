...
HEALTH_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE/api/health")
...
ALL_HEADERS=$(curl -s -I --max-time 10 "$BASE/")
FRAME_HEADER=$(echo "$ALL_HEADERS" | grep -i 'x-frame-options' | tr -d '\r' || true)
...
CSP_HEADER=$(echo "$ALL_HEADERS" | grep -i 'content-security-policy' | tr -d '\r' || true)
...
LIST_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE/api/bottles")
check "GET /api/bottles returns 2xx" "$(echo "$LIST_STATUS" | grep -qE '^2' && echo true || echo false)"
check "GET /api/bottles returns valid JSON array" "$(curl -s --max-time 10 "$BASE/api/bottles" | grep -qE '^\[' && echo true || echo false)"

# Test POST with missing name returns 422 (not 5xx)
MISSING_NAME_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" -X POST \
...
NOT_FOUND_STATUS=$(curl -s --max-time 10 -o /dev/null -w "%{http_code}" "$BASE/api/bottles/99999999")
...
NEW_STATUS=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/bottles/new")
...
NOTFOUND_EDIT=$(curl -s --max-time 15 -o /dev/null -w "%{http_code}" "$BASE/bottles/99999/edit")
